...
symbols_mapping: defaultdict[Symbol, list[Number]] = defaultdict(list)

for number in numbers_with_symbols:
    for symbol in number.symbols:
        symbols_mapping[symbol].append(number)

answer = 0
for symbol, numbers in symbols_mapping.items():
    if symbol.symbol == '*' and len(numbers) == 2:
